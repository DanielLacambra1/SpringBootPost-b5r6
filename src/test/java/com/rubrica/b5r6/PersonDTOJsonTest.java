...
public class PersonDTOJsonTest {
    ...
    private static final String PROFESSION = "Professional";
    private static final BigDecimal SALARY = BigDecimal.ZERO;

    private static final String JSON_TO_DESERIALIZE = "{\"firstName\":\"" + FIRST_NAME + "\",\"secondName\":\"" + SECOND_NAME + "\",\"dateOfBirth\":\""
        + DATE_OF_BIRTH_STRING + "\",\"profession\":\"" + PROFESSION + "\",\"salary\":" + SALARY + "}";
    ...
    public void firstNameDeserializes() throws IOException {
        assertThat(this.json.parseObject(JSON_TO_DESERIALIZE).getNombre()).isEqualTo(FIRST_NAME);
    }

    @Test
    public void secondNameDeserializes() throws IOException {
        assertThat(this.json.parseObject(JSON_TO_DESERIALIZE).getApellido()).isEqualTo(SECOND_NAME);
    }

    @Test
    public void dateOfBirthDeserializes() throws IOException {
        assertThat(this.json.parseObject(JSON_TO_DESERIALIZE).getFnac())
            .isEqualTo(DATE_OF_BIRTH);
    }

    @Test
    public void professionDeserializes() throws IOException {
        assertThat(this.json.parseObject(JSON_TO_DESERIALIZE).getProfesion()).isEqualTo(PROFESSION);
    }

    @Test
    public void salaryDeserializes() throws IOException {
        assertThat(this.json.parseObject(JSON_TO_DESERIALIZE).getSalario()).isEqualTo(SALARY);
    }
